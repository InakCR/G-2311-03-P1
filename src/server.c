...
int main(int argc, char *argv[]) {
        int socketServer,socketClient,err,ncliente;
        fd_set readfds;
        ...
        while (1) {
                ...
                err = pthread_create (&ptCliente[ncliente], &attr, deal_cliente,(void *) (intptr_t)socketClient);
                if(err < 0) on_error(LOG_ERR,"Error al crear hilo cliente");
                ncliente++;
        }
        return 0;
}
